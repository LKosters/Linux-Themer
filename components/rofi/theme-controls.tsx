...
function hexToRgba(hex: string, alpha: number): string {
  ...
  return alpha >= 1 ? hex : `rgba ( ${r}, ${g}, ${b}, ${Math.round(alpha * 100)} % )`
}

function generateRofiRasi(theme: RofiTheme): string {
  const bg = theme.windowOpacity < 1 ? hexToRgba(theme.windowBg, theme.windowOpacity) : theme.windowBg
...
function generateInstallScript(theme: RofiTheme): string {
  const rasi = generateRofiRasi(theme)

  return `#!/bin/bash
# Rofi Theme Install Script
...
# ── 1. Write theme file ────────────────────────────────
mkdir -p ~/.config/rofi
cat > ~/.config/rofi/theme.rasi << 'RASIEOF'
${rasi}RASIEOF
ok "Theme written to ~/.config/rofi/theme.rasi"

# ── 2. Update config to reference theme ────────────────
CONFIG_FILE="$HOME/.config/rofi/config.rasi"
if [ -f "$CONFIG_FILE" ]; then
  # Remove any existing @theme line
  sed -i '/@theme/d' "$CONFIG_FILE"
  echo '@theme "~/.config/rofi/theme.rasi"' >> "$CONFIG_FILE"
  ok "Updated existing config.rasi"
else
...
echo ""
echo "Test your theme with:"
echo "  rofi -show drun"
echo ""
echo "To revert:"
echo "  rm ~/.config/rofi/theme.rasi"
echo "  Remove the @theme line from ~/.config/rofi/config.rasi"
`
}
