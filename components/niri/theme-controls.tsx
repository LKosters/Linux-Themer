...
window-rule {
    ...
    clip-to-geometry true${theme.windowOpacity < 1 ? `
    opacity ${theme.windowOpacity}` : ""}
}

${theme.windowShadow ? `window-shadow {
    on
    color "${theme.shadowColor}80"
    softness ${theme.shadowSize}
}` : ""}
`
}
...
function generateWaybarConfig(theme: NiriTheme): string {
...
${conf}NIRIEOF
ok "Niri config written to ~/.config/niri/config.kdl"
...
# ── 3. Reload ──────────────────────────────────────────
# Niri auto-reloads config.kdl
niri msg action do-screen-transition 2>/dev/null && ok "Niri reloaded" || true
...
echo ""
echo "To revert:"
echo "  Restore your backup config.kdl"
`
}
