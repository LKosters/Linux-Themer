      ...
      <div className="border-t border-border px-4 py-3 space-y-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 rounded-lg py-2 text-sm font-sans font-medium transition-colors border border-border text-muted-foreground hover:text-foreground hover:bg-muted/50"
          >
            Config Only
          </button>
          <button
        ...
        <Link
          href="/docs/hyprland"
          className="block w-full rounded-lg py-2 text-sm font-sans font-medium text-center transition-colors text-muted-foreground/70 hover:text-foreground"
        >
          How to Install
        </Link>
      </div>
...
window#waybar {
    ...
    min-height: ${theme.barHeight}px;${theme.barBorderRadius > 0 ? `
...
function generateWaybarConfig(theme: HyprlandTheme): string {
  return JSON.stringify({
    layer: "top",
    position: theme.barPosition,
...
function generateInstallScript(theme: HyprlandTheme): string {
  const conf = generateHyprlandConf(theme)
  const waybarCss = generateWaybarCSS(theme)
  const waybarConfig = generateWaybarConfig(theme)

  return `#!/bin/bash
# Hyprland Theme Install Script
...
# ── 1. Hyprland config ──────────────────────────────────
mkdir -p ~/.config/hypr
cat > ~/.config/hypr/theme.conf << 'HYPREOF'
${conf}HYPREOF
ok "Hyprland theme config written to ~/.config/hypr/theme.conf"

# Source from main config if not already
if ! grep -q "source.*theme.conf" ~/.config/hypr/hyprland.conf 2>/dev/null; then
  echo 'source = ~/.config/hypr/theme.conf' >> ~/.config/hypr/hyprland.conf
  ok "Added source line to hyprland.conf"
else
  ok "hyprland.conf already sources theme.conf"
fi

# ── 2. Waybar config ───────────────────────────────────
mkdir -p ~/.config/waybar
cat > ~/.config/waybar/config.jsonc << 'WBCEOF'
${waybarConfig}
WBCEOF
ok "Waybar config written"

cat > ~/.config/waybar/style.css << 'WBSEOF'
${waybarCss}WBSEOF
ok "Waybar CSS written"
...
echo ""
echo "To revert:"
echo "  rm ~/.config/hypr/theme.conf"
echo "  Remove the 'source = ~/.config/hypr/theme.conf' line from hyprland.conf"
`
}
